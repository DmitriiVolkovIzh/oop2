...
public class Buses extends Cars {
    public enum SeatCapacity {
        SMALLEST(10), SMALL(11, 25),
        MIDDLE(40, 50), BIG(60, 80),
        BIGGEST(100, 120);
        ...
        public void setMaxNumberOfSeats(Integer maxNumberOfSeats) {
            if(maxNumberOfSeats <10)
                this.maxNumberOfSeats = 10;
            else this.maxNumberOfSeats = maxNumberOfSeats;
        }
        public void setMinNumberOfSeats(Integer minNumberOfSeats){
            if(minNumberOfSeats <3)
                this.minNumberOfSeats = 3;
            else this.minNumberOfSeats = minNumberOfSeats;
        }
        @Override
        public String toString() {
            if(getMinNumberOfSeats() != null)
                return "Вместимость: от " + minNumberOfSeats + " до " + maxNumberOfSeats;
            else return "Вместимость до: " + maxNumberOfSeats;
        }
    }
    static class DriverD extends Driver implements Skills{
    ...
    private final HashSet<Mechanic<? super Buses>> mechanics;
    public Buses(String brand, String model, double engineVolume, int seatCapacity,
    ...
    public void addSponsor(Sponsor sponsor, int amount){
    ...
    public void addMechanic(Mechanic<? super Buses> mechanic){
        Mechanic<Buses> mechanic1 = new Mechanic<>(mechanic.getFirstName(),
                mechanic.getLastName(), mechanic.getCompany());
        mechanics.add(mechanic1);
        mechanic.takeVehicle(this);
    }
    public void showAllMechanics(){
        for(Mechanic<? super Buses> mechanic: mechanics){
            System.out.println(mechanic);
        }
    }
    public HashSet<Mechanic<? super Buses>> getMechanics(){
        return mechanics;
    }
    ...
    public String participate() {
        return "Водитель " + driver.getName() + " управляет "
                + getBrand() + " и будет участвовать в соревнованиях";
    }
    @Override
    ...
    public String toString() {
        if(seatCapacity != null)
            return super.toString() + seatCapacity;
