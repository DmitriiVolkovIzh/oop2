...
public class Trucks extends Cars {
    ...
    static class DriverC extends Driver implements Skills{
    ...
    private final HashSet<Mechanic<? super Trucks>> mechanics;
    public Trucks(String brand, String model, double engineVolume, float capacity,
    ...
    public void showAllSponsors(){
        for(Sponsor sponsor: sponsors){
            System.out.println(sponsor + " проспонсировал заезд данного " + getBrand() + " на " + sponsor.getAmount() + " долларов");
        }
    }
    public void addMechanic(Mechanic<? super Trucks> mechanic){
        Mechanic<Trucks> mechanic1 = new Mechanic<>(mechanic.getFirstName(), mechanic.getLastName(), mechanic.getCompany());
        mechanics.add(mechanic1);
        mechanic.takeVehicle(this);
    }
    public void showAllMechanics(){
        for(Mechanic<? super Trucks> mechanic: mechanics){
            System.out.println(mechanic);
        }
    }
    public HashSet<Mechanic<? super Trucks>> getMechanics(){
        return mechanics;
    }
